...
class Settings {
    constructor() {
        ...
        this.defaults = {
            ...
            // DICOM integration settings (Pentax EPK-i8020c)
            dicomEnabled:  false,
            dicomAeTitle:  'MEDSCOPE',
            dicomPort:     4104,
            mwlPort:       4105,
            mppsPort:      4106,
            // Feature flags — set to false via Advanced Panel to disable report types
            featureEndoscopyReports: true,
            featureReferralReports:  true,
            featureOperationReports: true
        };
    ...
    saveSettings() {
        try {
            ...
            // Ensure save directories exist (only for string path values)
            Object.values(this.settings).forEach(value => {
